...
if(isset($_GET['ID']) && isset($_GET['title'])){
    ...
    $fetchNews = $connectToDb->prepare("SELECT * FROM slider WHERE id = :ID AND title = :title LIMIT 1");
    ...
    if($fetchNews->rowCount() == 1){
        foreach ($fetchNews as $row) {
            ...
            $whatcomment = "slider." .$newsTitle.".". $ID;
            ...
            //datum vijesti 
            $newsDate = new DateTime($row['date']);
            ...
            $numberOfNewsQuery = $connectToDb->query("SELECT id FROM slider ORDER BY id DESC LIMIT 1");
            $numberOfNews = 0;
            ...
            for ($i = $fetchNextNewsID; $i <= $numberOfNews; $i++) { 
                $fetchNextNews = $connectToDb->query("SELECT * FROM slider WHERE id = '$i' LIMIT 1");
                if($fetchNextNews->rowCount() == 1){
                    while ($r = $fetchNextNews->fetch(PDO::FETCH_OBJ)) {
                        $idOfNextNews = $r->id;
                        $titleOfNextNews = $r->title;
                    }
        ...
        // broj komentara
        $fetchComments = $connectToDb->query("SELECT * FROM comments WHERE slider_id = '$ID'");
                    ...
                    <div class="col-md-12">
                        <div class="blog-post-container">
                            <div class="blog-post-image">
                                <?php echo '<img src="./images/slider/'.$newsImage.'" alt="'.$newsImage.'" />'; ?>
